...
private:
  ...
  public:
    ...
    bool operator==(const pixel<T> &other) const = default;
  ...
  template <typename T> struct PixelHash {
  ...
  void process_uint8_pixels(const std::string &pixel_data, size_t total_pixels);
  void process_uint16_pixels(const std::string &pixel_data,
                             size_t total_pixels);
...
public:
  ...
  AOS(const std::vector<uint8_t> &binary);
