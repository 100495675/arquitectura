...
InputArgs::InputArgs(const std::vector<std::string> &argv_strings) {

  const int argc = static_cast<int>(argv_strings.size());
  ...
  const std::string &command_str = argv_strings[3];
  if (command_str == "info") {
...
void InputArgs::validate_info(int argc,
  ...
  if (argc != ARGC_INFO) {
    ...
    throw std::invalid_argument("Error: Invalid extra arguments for info: " +
                                extra_args);
  }
}

void InputArgs::validate_maxlevel(
    int argc, const std::vector<std::string> &argv_strings) {
  if (argc != ARGC_MAXLEVEL) {
    throw std::invalid_argument(
        "Error: Invalid number of extra arguments for maxlevel: " +
        std::to_string(argc - 4));
  }
  const std::string &level_str = argv_strings[POSITION_MAXLEVEL];
...
void InputArgs::validate_resize(int argc,
                                const std::vector<std::string> &argv_strings) {
  if (argc != ARGC_RESIZE) {
    throw std::invalid_argument(
        "Error: Invalid number of extra arguments for resize: " +
        std::to_string(argc - 4));
  }
  const std::string &width_str = argv_strings[POSITION_WIDTH];
  int width = 0;
  try {
  ...
  const std::string &height_str = argv_strings[POSITION_HEIGHT];
...
void InputArgs::validate_cutfreq(int argc,
                                 const std::vector<std::string> &argv_strings) {
  if (argc != ARGC_CUTFREQ) {
    throw std::invalid_argument(
        "Error: Invalid number of extra arguments for cutfreq: " +
        std::to_string(argc - 4));
  }
  const std::string &freq_str = argv_strings[POSITION_FREQ];
...
void InputArgs::validate_compress(
    int argc, const std::vector<std::string> &argv_strings) {
  if (argc != ARGC_COMPRESS) {
    ...
    throw std::invalid_argument(
        "Error: Invalid extra arguments for compress: " + extra_args);
  }
}
...
std::string InputArgs::getInput() const { return input; }
std::string InputArgs::getOutput() const { return output; }
InputArgs::Command InputArgs::getCommand() const { return command; }
