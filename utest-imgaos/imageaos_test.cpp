...
namespace imgaos {
  ...
  TEST(imgaos, normal_uint8) {
    ...
    std::vector<AOS::pixel<uint8_t>> const expected = {
      {0x01, 0x02, 0x03},
      {0x04, 0x05, 0x06},
      {0x07, 0x08, 0x09},
      {0x0A, 0x0B, 0x0C},
    ...
    ASSERT_EQ(expected, std::get<std::vector<AOS::pixel<uint8_t>>>(img.data));
  }
  ...
  TEST(imgaos, normal_uint16) {
    ...
    std::vector<AOS::pixel<uint16_t>> const expected = {
    ...
    auto const & pixel_vector = std::get<std::vector<AOS::pixel<uint16_t>>>(img.data);
