...
TEST(imgaos, normal_uint8) {
  ...
  const std::vector<AOS::pixel<uint8_t>> expected = {
...
TEST(imgaos, normal_uint16) {
  ...
  const std::vector<AOS::pixel<uint16_t>> expected = {
      {0x0102, 0x0304, 0x0506}, {0x0708, 0x090A, 0x0B0C},
      {0x0D0E, 0x0F10, 0x1112}, {0x1314, 0x1516, 0x1718},
      {0x191A, 0x1B1C, 0x1D1E}, {0x1F20, 0x2122, 0x2324},
      {0x2526, 0x2728, 0x292A}, {0x2B2C, 0x2D2E, 0x2F30},
  };
  const auto &pixel_vector =
      std::get<std::vector<AOS::pixel<uint16_t>>>(img.data);
